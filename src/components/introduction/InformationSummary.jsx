...
const InformationSummary = ({item}) => {
    ...
    return (
        <div className={`${lightTheme ?'bg-[#F6EBFE]':'bg-[#323d4c]'} mx-0.5 text-center w-[205px] h-[80px]`}>
            <p className={`text-[32px] font-semibold ${lightTheme?'text-gray-700':'text-amber-10'}`}>{item.description}</p>
            <p className={`text-[16px] ${lightTheme?'text-gray-500':'text-gray-300'}`}>{item.title}</p>
        </div>
    );
};
